...
class LaserScan:
    ...
    def reset(self):
        ...
        self.image_rgb = np.zeros((0, 3), dtype=np.float32)  # [m ,1]: remission
        ...
        # projected rgb - [H,W] intensity (-1 is no data)
        self.proj_r = np.full((self.proj_H, self.proj_W), 0,
                                      dtype=np.uint8)
        self.proj_g = np.full((self.proj_H, self.proj_W), 0,
                              dtype=np.uint8)
        self.proj_b = np.full((self.proj_H, self.proj_W), 0,
                              dtype=np.uint8)
    ...
    def open_scan(self, filename):
        ...
        if self.drop_points is not False:
            ...
            R = np.delete(R,self.points_to_drop,axis=0)
            G = np.delete(G, self.points_to_drop, axis=0)
            B = np.delete(B, self.points_to_drop, axis=0)

        self.set_points(points, R, G, B)

    def set_points(self, points, R, G, B):
        ...
        # check RGB makes sense
        for color in R, G, B:
            if color is not None and not isinstance(color, np.ndarray):
                raise TypeError("RGB Channel should be numpy array")
        ...
        if color is not None:
            ...
            self.R = np.zeros((points.shape[0]), dtype=np.uint8)
            self.G  = np.zeros((points.shape[0]), dtype=np.uint8)
            self.B  = np.zeros((points.shape[0]), dtype=np.np.uint8)
    ...
    def do_range_projection(self):
        ...
        self.proj_r[proj_y, proj_x] = self.R[order]
        self.proj_g[proj_y, proj_x] = self.G[order]
        self.proj_b[proj_y, proj_x] = self.B[order]
        self.proj_mask = (self.proj_idx > 0).astype(np.int32)

        #show image
        proj = np.zeros([64, 2048, 3], dtype=np.uint8)
        proj[:, :, 0] = self.proj_r
        proj[:, :, 1] = self.proj_g
        proj[:, :, 2] = self.proj_b
