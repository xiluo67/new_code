...
class SemanticKitti(Dataset):
  ...
  def __init__(self, root,    # directory where data is
    ...
    for seq in self.sequences:
      # to string
        print("parsing seq here {}".format(seq))
      ...
      # get files
        scan_files = [os.path.join(dp, f) for dp, dn, fn in os.walk(
          os.path.expanduser(scan_path)) if "Annotations" in dp
          for f in fn if is_scan(f)]
        label_files = [os.path.join(dp, f) for dp, dn, fn in os.walk(
          os.path.expanduser(scan_path)) if "label" in dp
    ...
    #     label_path = scan_file.rsplit("/", 2)[0] + "/label"
    #     label_name = scan_file.rsplit("/", 1)[1].split(".txt", 1)[0] + ".label"
    #
    #     os.makedirs(label_path, exist_ok=True)
    #     for key, data in self.labels.items():
    #         if data in label_name:
    #             points = np.loadtxt(scan_file, dtype=np.int32)
    #             label_value = np.full(points.shape[0], key, dtype=np.int32)
    #             os.chdir(label_path)
    #             print(label_path)
    #             print(label_value)
    #             np.savetxt(label_name, label_value)
  ...
  def __getitem__(self, index):
    ...
    proj_r = torch.from_numpy(scan.proj_r).clone()
    proj_g = torch.from_numpy(scan.proj_g).clone()
    proj_b = torch.from_numpy(scan.proj_b).clone()
    ...
    proj = torch.cat([proj_range.unsqueeze(0).clone(),
                      proj_r.unsqueeze(0).clone(),
                      proj_g.unsqueeze(0).clone(),
                      proj_b.unsqueeze(0).clone(),
    ...
    path_name = path_split[-1].replace(".txt", ".label")

    # return
    return proj, proj_mask, proj_labels, unproj_labels, path_seq, path_name, proj_x, proj_y, proj_range, unproj_range, proj_xyz, unproj_n_points
